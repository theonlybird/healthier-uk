/**
 * POST /api/submit — the contributor holding bay (Netlify build).
 *
 * Identical behaviour to functions/api/submit.js (the Cloudflare version):
 * takes a submission from contribute.html and writes it into the repository as an
 * UNPUBLISHED draft. Nothing here can put anything on the live site — every file it
 * writes carries `published: false`, and Eleventy filters those out at build time.
 *
...
const decodeBase64 = (b64) => Buffer.from(b64.replace(/\n/g, ''), 'base64').toString('utf8');
...
export default async function handler(request) {
  ...
  try {
    ...
    const ok = await verifyTurnstile(
      env, field('cf-turnstile-response'),
