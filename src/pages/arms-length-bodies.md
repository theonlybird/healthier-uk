---
title: Arms length bodies
heading: ''
lead: Aligning national agencies across housing, nature, transport, and sports to create the conditions for population health.
leadIn: ''
image: ''
imageAlt: ''
callout: <strong>ALB Working Group:</strong> Are you working in a national agency or public body interested in integrating health creation principles into your policy design? <a href="contact.html">Connect with our policy leads</a>.
ctaBox: null
featureCard: null
cardsHeading: ''
cards: []
backHref: england.html
backLabel: Back to England
nextHref: combined-authorities.html
nextLabel: Combined authorities →
description: Engaging UK Arms length bodies to embed health creation into national infrastructure, housing, environment, and planning.
section: about
introHeading: ''
introImage: ''
introImageAlt: ''
breadcrumbs:
  - label: England
    url: england.html
---

## Cross-government health creation

##### 

Health is shaped far more by decisions made outside the Department of Health and Social Care (DHSC) than within it. As a result from the Parliamentary Launch of Healthier UK, six  Arms Length Bodies are now working together to pool resources, share knowledge and work together in the places where they have co-located projects.

![Natural infrastructure and green spaces](assets/images/feature-nature.jpeg)

## **Arms Length Body collaboration**

- **Homes England:** As the government’s housing and regeneration agency we believe that affordable, quality homes in well-designed places are key to improving people’s lives. Together with our partners, we’re accelerating the pace of house building and regeneration across the country.
- **Active Travel England:** "Our vision is for walking, wheeling and cycling to be an affordable, safe, and accessible choice for everyone. Our goal is to ensure that more people are making that choice every day".
- **Natural England:** "Our purpose is to help conserve, enhance and manage the natural environment for the benefit of present and future generations, thereby contributing to sustainable development".
- **Sport England:** "We’re here to invest in sport and physical activity to make it a normal part of life for everyone in England, regardless of who you are".
- **Arts Council England:** "We are the national development agency for creativity and culture. We help people in every corner of the country to experience and benefit from creativity".
- **Historic England:** "We are the public body that helps people care for, enjoy and celebrate England's spectacular historic environment".
