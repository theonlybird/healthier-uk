/**
 * POST /api/submit-casebook — the Casebook holding bay.
 *
 * Takes a case study from submit-casebook.html and writes it into the repository
 * as an UNPUBLISHED entry. Like /api/submit it can never put anything on the live
 * site: every file it writes carries `published: false`, and both the collection
 * filter and the computed permalink in casebook.11tydata.js keep it out of the
 * build until an editor turns it on at /admin.
 *
 * Deliberately different from the blog submission in one way: this writes ONE
 * file and touches nothing else. No team.json, no organisations.json. The
 * Casebook is expected to run to hundreds of entries, and threading each of them
 * through the curated people and organisation records would turn two small,
 * hand-tended files into a dumping ground — and make every submission a
 * read-modify-write race on the same two files. The organisation and the contact
 * live in the entry itself, as free text.
 *
